...
快取在與超執行緒、核心、和處理器的關係中的位置並不在程式設計師的控制之下。但程式設計師能夠決定執行緒要在何處執行，於是快取如何與使用的 CPU 共處就變得很重要了。

這裡我們不會深入在何時選擇哪顆核心來執行執行緒的細節。我們只會描述在設置執行緒的親和性（affinity）時，程式設計師必須要考慮的架構細節。
...
對程式設計師而言，所有這些不同的設計都代表進行排程決策時的複雜性。為了達到最好的效能，必須知道工作負載以及機器架構的細節。幸運的是，我們擁有確定機器架構的依據。這些介面會在之後的章節中介紹。
