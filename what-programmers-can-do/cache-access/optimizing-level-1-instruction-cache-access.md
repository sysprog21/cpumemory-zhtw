...
行內展開也能被個別處理。編譯器擁有引導行內展開的啟發法（heuristic）與限制；這些限制能夠由程式開發者控制。`-finlinelimit` 選項指定對行內展開而言，必須被視為過大的函式有多大。若是一個函式在多處被呼叫，在所有函式中行內展開它便會導致程式大小的劇增。但還有更多細節。假設一個函式 `inlcand` 在二個函式 `f1` 與 `f2` 中被呼叫。函式 `f1` 與 `f2` 本身是先後被呼叫的。
